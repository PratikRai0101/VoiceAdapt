"""
python/vad.py - Voice Activity Detection
Segments mic audio into utterance chunks using Silero VAD.
Each chunk gets a UUID that tracks it through the whole pipeline.
"""
...
class VADSegmenter:
    ...
    def _load_model(self):
        ...
        console.print('[dim]Loading Silero VAD...[/dim]')
        import torch
    ...
    def stream_mic(self) -> Generator[AudioChunk, None, None]:
        ...
        with sd.InputStream(samplerate=SAMPLE_RATE, channels=1,
                            ...
                            device=self.device, callback=self._mic_callback):
            while True:
                try:
                ...
                if is_speech:
                    ...
                    if len(utterance_buf) >= max_frames:
                        ...
                        if np.concatenate(utterance_buf).shape[0]/SAMPLE_RATE*1000 >= MIN_SPEECH_MS:
                            yield self._emit(utterance_buf)
                        utterance_buf, silence_frames, in_speech = [], 0, False
    ...
    def _emit(self, frames) -> AudioChunk:
        ...
        console.print(f'[cyan]chunk[/cyan] {chunk.uuid[:8]}  [dim]{chunk.duration_s:.2f}s[/dim]')
        return chunk


def segment_file(audio_path: str, threshold=0.5) -> list:
    ...
    if sr != SAMPLE_RATE:
        ...
        audio   = torchaudio.functional.resample(audio_t, sr, SAMPLE_RATE).squeeze(0).numpy()
    model, utils = torch.hub.load('snakers4/silero-vad','silero_vad',force_reload=False,onnx=False)
    get_ts = utils[0]
    timestamps = get_ts(torch.from_numpy(audio), model, sampling_rate=SAMPLE_RATE,
    ...
    return [AudioChunk(audio=audio[ts['start']:ts['end']]) for ts in timestamps]
...
if __name__ == '__main__':
    ...
    try:
        for chunk in seg.stream_mic():
            console.print(f'  {chunk.uuid[:8]}  {chunk.duration_s:.2f}s')
